...
export type GeneratePlanState = { error: string | null };
...
export async function generatePlan(
  ...
  if (!limits.research) {
    return { error: "Content planning is a Pro feature. Upgrade to use it." };
  }
  ...
  if (platforms.length === 0) {
    return { error: "Connect at least one active account before generating a plan." };
  }
